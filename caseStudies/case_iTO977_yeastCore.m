function [OriginalGEM, GEMmodel, core_ss, Biomass_rxns, met_pairs_to_remove, InorgMetSEEDIDs, BBBsToExclude, ExtraCellSubsystem] = case_iTO977_yeastCore(ZeroZeroGEMbounds, FluxUnits, ListForInorganicMets, ListForCofactorPairs, SelectedSubsystems, AddExtracellularSubsystem, DB_AlbertyUpdate)

% Model Specific Settings:
fprintf('Loading the GEM for YeastCore iTO977 ...\n')
GEM_filename = 'iTO977_yeastCore.mat';
GetModelFromGITresources(GEM_filename)
GEMmodel = load(['./GEMs/',GEM_filename]);
GEMmodel = GEMmodel.iTO_yeastCore;
...
elseif ischar(AddExtracellularSubsystem) && strcmp(AddExtracellularSubsystem,'default')
    fprintf('- The extracellular subsystm includes the following reactions:\n')
    ExtraCellSubsystem = {'SUCCxtI'; % 'succ_e <=> '
                          'ACxtI';   % 'ac_e <=> ' 
                          'ETHxtI';
                          'GLxtI';
                          'LACxtI';
...
Biomass_rxns = {'cholestenol delta-isomerase, lumped reaction';...
    'lipid pseudoreaction';...
    'yeast 5 biomass pseudoreaction';...
    'yeast 6 biomass pseudoreaction';...
    'yeast 8 biomass pseudoreaction'};
...
GEMmodel = prepModelforTFBA(GEMmodel,DB_AlbertyUpdate,GEMmodel.CompartmentData);
