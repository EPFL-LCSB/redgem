function [OriginalGEM, GEMmodel, core_ss, Biomass_rxns, met_pairs_to_remove, InorgMetSEEDIDs, BBBsToExclude, ExtraCellSubsystem] = case_falciparum(ZeroZeroGEMbounds, FluxUnits, ListForInorganicMets, ListForCofactorPairs, SelectedSubsystems, AddExtracellularSubsystem, DB_AlbertyUpdate)

% Model Specific Settings:
fprintf('Loading the GEM for P. falciparum...\n')
GEM_filename = 'P_falciparum.mat';
...
elseif ischar(SelectedSubsystems) && strcmp(SelectedSubsystems, 'default')
    core_ss = {'Citrate cycle (TCA cycle)'
        'ETC'
...
elseif ischar(AddExtracellularSubsystem) && strcmp(AddExtracellularSubsystem,'default')
    ...
    %         'DM_akg_e'
    %         'DM_mal_L_e'}
    error('ExtraCellSubsystem not yet defined!')
...
Biomass_rxns = {'Biomass_rxn_c'}
ObjRxn = {'Biomass_rxn_c'};
...
if strcmp(ListForCofactorPairs,'curated')
    % load curated list from mat-file
    met_pairs_to_remove = load('met_pairs_to_remove_falciparum.mat');
