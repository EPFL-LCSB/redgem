function [OriginalGEM, GEMmodel, core_ss, Biomass_rxns, met_pairs_to_remove, InorgMetSEEDIDs, BBBsToExclude, ExtraCellSubsystem] = case_yeast7_yeastCore(ZeroZeroGEMbounds, FluxUnits, ListForInorganicMets, ListForCofactorPairs, SelectedSubsystems, AddExtracellularSubsystem, DB_AlbertyUpdate)

% Model Specific Settings:
fprintf('Loading the GEM for YeastCore Yeast7 ...\n')
GEM_filename = 'Y7_yeastCore.mat';
GetModelFromGITresources(GEM_filename)
GEMmodel = load(['./GEMs/',GEM_filename]);
GEMmodel = GEMmodel.Y7_yeastCore;
...
elseif ischar(AddExtracellularSubsystem) && strcmp(AddExtracellularSubsystem,'default')
    fprintf('- The extracellular subsystm includes the following reactions:\n')
    ExtraCellSubsystem = {'r_2056'; % 'succ_e <=> '
                          'r_1634'; % 'ac_e <=> '   
                          'r_1761';
                          'r_1808'; % 'glyc_e <=> ' 
                          'r_1546'; % 'lac_D_e <=> '
